...
def main(args=None):
    ...
    model = TD3Obstacle(
        ...
        # load_model_name="TD3-MARL-obstacle-14robots",
        load_directory=Path("robot_nav/models/MARL/marlTD3/checkpoint/Mar.15_obstacle_14robot_reward8"),
    ...
    for episode in range(max_episodes):
        ...
        while steps < max_steps:
            ...
            # Print instant rewards for each robot (condensed for 14 robots)
            if steps % 10 == 0 or any(collision) or any(goal):
        ...
        # Episode summary
        if (episode + 1) % 10 == 0 or episode == max_episodes - 1:
            avg_reward = np.mean(episode_total_rewards[-10:]) if len(episode_total_rewards) >= 10 else np.mean(episode_total_rewards)
