"""
Training script for MARL TD3 with 14 Robots — WITHOUT ACTION STATE (ablation).

Ablation experiment: trains the same 14-robot MARL TD3 policy but with
linear and angular velocity removed from robot state, node features, and
edge features. Compare against marl_train_obstacle_14robots.py to measure
the impact of action information on obstacle avoidance performance.

Key differences from marl_train_obstacle_14robots.py:
- state_dim = 9 (no lin_vel, ang_vel)
- Uses marlTD3_obstacle_woact (iga_obstacle_woactstate_optimized backbone)
"""
...
from robot_nav.models.MARL.marlTD3.marlTD3_obstacle_woact import TD3Obstacle
...
def main(args=None):
    ...
    action_dim = 2
    max_action = 1
    state_dim = 9  # Robot state dimension (no lin_vel, ang_vel)
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING - 14 ROBOTS (WITHOUT ACTION STATE)")
    print(f"{'='*60}")
    ...
    model = TD3Obstacle(
        ...
        load_directory=Path("robot_nav/models/MARL/marlTD3/checkpoint/Mar.15_obstacle_14robot_woact"),
        model_name="TD3-MARL-obstacle-14robots-woact",
        save_directory=Path("robot_nav/models/MARL/marlTD3/checkpoint/Mar.15_obstacle_14robot_woact"),
    )
