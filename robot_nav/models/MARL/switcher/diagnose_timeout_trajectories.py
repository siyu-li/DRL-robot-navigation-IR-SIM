...
Usage:
    python -m robot_nav.models.MARL.switcher.diagnose_timeout_trajectories
"""
...
class OracleEvaluator:
    ...
    # ----- action coupling (identical to allrobotreach) -----
    def get_action_for_group(self, robot_obs, obstacle_obs, group):
        ...
        use_rot = CONFIG.get("use_rotation_coupling", True)
        ...
        if group_size == 1:
            ...
            for i in range(self.num_robots):
                if i == robot_idx:
                    a_out.append([(action[i][0] + 1) / 4, action[i][1]])
                else:
                    a_out.append([0.0, 0.0])
            return a_out, action  # also return raw policy action
        else:
            scaled_lin_vels = [(action[idx][0] + 1) / 4 for idx in group]
            v_coupled = min(scaled_lin_vels)
            if group_size > 3 and use_rot:
