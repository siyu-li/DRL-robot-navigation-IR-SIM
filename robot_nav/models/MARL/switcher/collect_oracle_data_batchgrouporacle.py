...
class OracleDataCollector:
    ...
    def _evaluate_all_groups_batched(
        ...
        This reduces GPU calls from (N_groups × horizon) to just (horizon) calls,
        a ~41x reduction in GPU forward passes.
    ...
    def _evaluate_group(
        ...
        for rollout_idx in range(self.n_rollouts_per_group):
            # For single-group fallback, use the sequential method
            # (restore snapshot, step sim, get actions per step)
            had_collision = False
            had_goal = False
    ...
    def collect_sample(
        ...
        3. Evaluate ALL groups using batched rollouts:
           - At each horizon step, batch all active groups' observations 
             into a SINGLE GPU forward pass (1 call instead of N_groups calls)
           - Reduces GPU calls from N_groups×horizon to just horizon per sample
        4. Return sample dict
        
        GPU call reduction: 41×10 = 410 → 10 calls (one batched call per step)
        This is a ~41x reduction in GPU forward passes!
        ...
        # Get extra features (CPU, fast)
        extra = self._get_extra_features(poses, distance, goal_positions)
        
        # === BATCHED GROUP EVALUATION ===
        # Uses batched GPU inference: all groups' observations per step
    ...
    def collect_dataset(
        ...
        for i in pbar:
            ...
            # The collect_sample method precomputes raw actions once (1 GPU call)
            # and reuses them across all 41 groups for step 0
            ...
            # GPU calls per sample (batched mode):
            #   1 (embeddings) + horizon batched calls ≈ 11 total
            # vs original: 41 groups × 10 steps + 1 embed = 411
            # Savings: ~40x fewer GPU forward passes!
            gpu_calls += 1 + self.horizon  # embedding + batched steps
            
            # Take action from a randomly selected group to advance simulation
            # Reuse the prepare_state + get_raw_actions pattern
            robot_state, _ = self.policy.prepare_state(
                poses, distance, cos, sin, collision, action, goal_positions
            )
            robot_obs = np.array(robot_state)
            
            # Get raw actions once for advancement step
            raw_actions = self.get_raw_actions(robot_obs, obstacle_states)
            gpu_calls += 1
            
            # Randomly select a group and derive action from cached raw output
            random_group = random.choice(self.groups)
            scaled_action = self.actions_for_group_from_raw(raw_actions, random_group)
        ...
        data = {
            ...
            "config": {
                ...
                "collection_method": "simulation_rollout_batch_optimized",
...
def main():
    ...
    print("=" * 70)
    print("Oracle Data Collection for Group Switcher (Batch-Optimized GPU)")
    print("=" * 70)
