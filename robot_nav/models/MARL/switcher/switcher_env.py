...
class SwitcherEnv:
    ...
    def step(
        ...
        with torch.no_grad():
            for inner_step in range(self.selection_interval):
                ...
                action_out = self._actions_for_group(raw_actions, group)
    ...
    def _actions_for_group(
        self, raw_actions: np.ndarray, group: List[int],
    ) -> List[List[float]]:
        """Derive coupled actions from raw policy output (CPU only)."""
        group_size = len(group)
        group_set = set(group)

        # Scaled linear velocities for group members
        scaled = [(raw_actions[i][0] + 1) / 4 for i in group]  # [-1,1]→[0,0.5]
        v_coupled = min(scaled)

        # Angular velocity coupling
        if (
            group_size > self.rotation_coupling_threshold
            and self.use_rotation_coupling
        ):
            w_coupled = float(np.mean([raw_actions[i][1] for i in group]))
        else:
