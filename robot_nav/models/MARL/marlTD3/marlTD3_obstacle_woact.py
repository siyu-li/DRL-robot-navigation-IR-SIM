"""
TD3 Multi-Agent RL with Obstacle Graph Nodes (WITHOUT ACTION STATE).

Ablation variant of marlTD3_obstacle.py that removes linear and angular
velocity from both the robot node features and robot-robot edge features.
Uses iga_obstacle_woactstate_optimized as the attention backbone.

State vector is 9-dim (vs 11-dim in the original):
  [px, py, cos_h, sin_h, dist/17, cos_goal, sin_goal, gx, gy]

Key differences from marlTD3_obstacle.py:
- Uses AttentionObstacleOptimized from iga_obstacle_woactstate_optimized
- Robot state is 9-dim (no lin_vel, ang_vel)
- GAT node features are 3-dim (no action)
- GAT robot-robot edge features are 5-dim (no action)
"""
...
from robot_nav.models.MARL.Attention.iga_obstacle_woactstate_optimized import AttentionObstacleOptimized as AttentionObstacle
...
class TD3Obstacle:
    ...
    def prepare_state(
        ...
        for i in range(self.num_robots):
            ...
            # No lin_vel / ang_vel — this is the woact ablation
            state = [
                px,
                py,
                heading_cos,
