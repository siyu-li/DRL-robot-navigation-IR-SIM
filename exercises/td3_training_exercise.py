...
Run command:
    python -m exercises.td3_training_exercise
...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int):
        ...
        # TODO-4: Define layers for Q1 network.
        #         Name them: q1_l1, q1_l2_s, q1_l2_a, q1_l3, q1_l4
        #         q1_l1:   Linear(state_dim → 256)
        #         q1_l2_s: Linear(256 → 256)        ← state stream after first layer
        #         q1_l2_a: Linear(action_dim → 256)  ← action stream merged here
        #         q1_l3:   Linear(256 → 256)
        #         q1_l4:   Linear(256 → 1)           ← scalar Q-value
        # -----------------------------------------------------------------------
        # Q1 network
        ...
        # Q2 network  (identical structure, separate weights)
        ...
        # Apply Kaiming initialisation to all layers
    ...
    def forward(
        ...
        """
        # TODO-5: Implement the Q1 forward pass, then repeat for Q2.
        #
        #         Q1 pass:
        ...
        # -----------------------------------------------------------------------
        # Q1
        s1 = F.leaky_relu(self.q1_l1(state))
        s1 = F.leaky_relu(self.q1_l2_s(s1) + self.q1_l2_a(action))
        s1 = F.leaky_relu(self.q1_l3(s1))
        q1 = self.q1_l4(s1)

        # Q2
        s2 = F.leaky_relu(self.q2_l1(state))
        s2 = F.leaky_relu(self.q2_l2_s(s2) + self.q2_l2_a(action))
        s2 = F.leaky_relu(self.q2_l3(s2))
        q2 = self.q2_l4(s2)
        # -----------------------------------------------------------------------
        return q1, q2
...
class ReplayBuffer:
    ...
    When full, the oldest transition is automatically discarded (popleft).
    """
    ...
    def sample(self, batch_size: int):
        ...
        TODO-6: Implement this method.
                ...
                3. Convert each list to a numpy array with np.array(...).
                4. Return (states, actions, rewards, dones, next_states)
                   as numpy arrays with dtype float32.
...
class TD3Agent:
    ...
    def select_action(self, state: np.ndarray, add_noise: bool = True) -> np.ndarray:
        ...
        Steps:
          1. Convert numpy state → torch tensor on self.device
          2. Run actor in no-grad mode
          3. Convert output tensor back to numpy (cpu().numpy())
          4. Optionally add Gaussian exploration noise
        ...
        if add_noise:
            ...
            action_np = np.clip(action_np + noise, -self.max_action, self.max_action)
    ...
    def train_step(
        ...
        #          .view() reshapes without copying memory (like numpy reshape).
        ...
        # -----------------------------------------------------------------------
        s   = torch.FloatTensor(states).to(self.device)       # (B, state_dim)
        a   = torch.FloatTensor(actions).to(self.device)      # (B, action_dim)
        r   = torch.FloatTensor(rewards).to(self.device)      # (B, 1)
        d   = torch.FloatTensor(dones).to(self.device)        # (B, 1)  — done flag
        s_  = torch.FloatTensor(next_states).to(self.device)  # (B, state_dim)
        # -----------------------------------------------------------------------
        ...
        #          ("detached from the graph"), not as something to differentiate.
        #
        #          Bellman equation:
        ...
        with torch.no_grad():
            ...
            #         2. noise  = normal tensor clipped to [-noise_clip, noise_clip]
            #         3. next_a = clamp(next_a + noise, -max_action, max_action)
            # -------------------------------------------------------------------
            next_a = self.actor_target(s_)
            noise  = (torch.randn_like(next_a) * policy_noise).clamp(-noise_clip, noise_clip)
            next_a = (next_a + noise).clamp(-self.max_action, self.max_action)
            # -------------------------------------------------------------------

            # TODO-10: Compute the Bellman target Q value.
            #          1. Get (Q1_next, Q2_next) from critic_target(s_, next_a)
            #          2. target_Q = r + (1 - d) * discount * min(Q1_next, Q2_next)
            # -------------------------------------------------------------------
            Q1_next, Q2_next = self.critic_target(s_, next_a)
            target_Q = r + (1.0 - d) * discount * torch.min(Q1_next, Q2_next)
            # -------------------------------------------------------------------
        ...
        # TODO-11: Apply the standard update pattern to self.critic_optimizer.
        ...
        if self.iter_count % policy_freq == 0:
            ...
            # TODO-12: Compute actor_loss = -mean of Q1(s, actor(s)).
            # -------------------------------------------------------------------
            ...
            actor_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.actor.parameters(), 7.0)
            ...
            # This is much more stable than copying weights every N steps.

            # TODO-14: Implement soft update for BOTH actor and critic target networks.
            #          Iterate over zip(network.parameters(), target.parameters())
            #          and use target_param.data.copy_( ... )
            # -------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------
        self.writer.add_scalar("train/critic_loss", critic_loss.item(), self.iter_count)
        if actor_loss is not None:
            self.writer.add_scalar("train/actor_loss", actor_loss.item(), self.iter_count)
        # -----------------------------------------------------------------------
    ...
    #          We save the state_dicts of all four networks + iter_count.
    #          On load, map_location=self.device ensures GPU checkpoints can be
    #          loaded on a CPU machine (common when moving from server to laptop).
    ...
    def save(self, name: str = "checkpoint") -> None:
        ...
        TODO-16: Use torch.save() to save each network's state_dict.
                 ...
                 Also save self.iter_count so training can resume from the correct step.
        ...
        # -----------------------------------------------------------------------
        torch.save(self.actor.state_dict(),         self.save_dir / f"{name}_actor.pth")
        torch.save(self.actor_target.state_dict(),  self.save_dir / f"{name}_actor_target.pth")
        torch.save(self.critic.state_dict(),        self.save_dir / f"{name}_critic.pth")
        torch.save(self.critic_target.state_dict(), self.save_dir / f"{name}_critic_target.pth")
        torch.save({"iter_count": self.iter_count}, self.save_dir / f"{name}_meta.pth")
        print(f"  Saved checkpoint: {self.save_dir / name}")
        # -----------------------------------------------------------------------
    ...
    def load(self, name: str = "checkpoint") -> None:
        ...
        TODO-17: Mirror save() using torch.load() and load_state_dict().
                 Always pass map_location=self.device to torch.load().
        """
        # -----------------------------------------------------------------------
        self.actor.load_state_dict(
            torch.load(self.save_dir / f"{name}_actor.pth", map_location=self.device))
        self.actor_target.load_state_dict(
            torch.load(self.save_dir / f"{name}_actor_target.pth", map_location=self.device))
        self.critic.load_state_dict(
            torch.load(self.save_dir / f"{name}_critic.pth", map_location=self.device))
        self.critic_target.load_state_dict(
            torch.load(self.save_dir / f"{name}_critic_target.pth", map_location=self.device))
        meta = torch.load(self.save_dir / f"{name}_meta.pth", map_location=self.device)
        self.iter_count = meta["iter_count"]
        print(f"  Loaded checkpoint: {self.save_dir / name}  (iter {self.iter_count})")
        # -----------------------------------------------------------------------
...
def main():
    ...
    # =========================================================================
    # TODO-18: Understand (then optionally modify) the training loop below.
    #          Trace through ONE full iteration:
    #            a) agent.select_action is called with the current state
    #            b) env.step returns (next_state, reward, done)
    #            c) buffer.add stores the transition
    #            d) on episode end, agent.train_step is called
    # =========================================================================
    ...
    while epoch < MAX_EPOCHS:
        ...
        if done or steps >= MAX_STEPS:
            ...
            if episode_count >= TRAIN_EVERY_N and len(buffer) >= BATCH_SIZE:
                ...
                # Compute episode-level metrics for logging
                avg_reward = running_reward / max(running_timesteps, 1)

                # TODO-19: Log "run/avg_reward" to agent.writer using add_scalar.
                # -------------------------------------------------------------------
                agent.writer.add_scalar("run/avg_reward", avg_reward, agent.iter_count)
                agent.writer.add_scalar("run/buffer_size", len(buffer), agent.iter_count)
                # -------------------------------------------------------------------
